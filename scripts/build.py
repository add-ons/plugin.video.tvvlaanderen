...
if __name__ == '__main__':
    ...
    if sys.argv[-1] == 'base':
        ...
        for brand in [o for o in os.listdir(BRANDS_DIR) if os.path.isdir(os.path.join(BRANDS_DIR, o))]:
            dest = os.path.join(DIST_DIR, brand)
            if not os.path.isdir(dest):
                os.mkdir(dest)
            ...
            # Copy files from brand
            shutil.copytree(os.path.join(BRANDS_DIR, brand), dest, dirs_exist_ok=True)
